...
def test_upload_media_webp_and_blur_hash(dummy_image):
    # Upload photo
    ...
    response = client.post("/api/v1/media", data=data, files=files)
...
def test_primary_photo_constraints(dummy_image):
    ...
    resp1 = client.post("/api/v1/media", data=data1, files=files1)
    id1 = resp1.json()["id"]
    ...
    resp2 = client.post("/api/v1/media", data=data2, files=files2)
    id2 = resp2.json()["id"]
    assert resp2.json()["is_primary"] is False

    # Promote Photo 2 to primary
    promote_resp = client.put(f"/api/v1/media/{id2}/primary")
...
def test_auto_reassign_primary_on_deletion(dummy_image):
    ...
    resp1 = client.post("/api/v1/media", data={"owner_type": "hotel", "owner_id": "MOCK_HT_3", "alt_text": "P1"}, files=files1)
    id1 = resp1.json()["id"]

    files2 = {"file": ("p2.gif", BytesIO(dummy_image), "image/gif")}
    resp2 = client.post("/api/v1/media", data={"owner_type": "hotel", "owner_id": "MOCK_HT_3", "alt_text": "P2"}, files=files2)
    id2 = resp2.json()["id"]

    # Delete primary Photo 1
    del_resp = client.delete(f"/api/v1/media/{id1}")
