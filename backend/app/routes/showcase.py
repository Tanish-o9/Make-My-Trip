...
def seed_showcase_data(db: Session):
    try:
        ...
        db.rollback()
        logger.error(f"Error seeding showcase data: {e}", exc_info=True)
...
def get_footer_data(db: Session = Depends(get_db)):
    try:
        ...
        logger.error(f"Failed to seed showcase data in get_footer_data: {e}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"Failed to query or process footer data: {e}", exc_info=True)
        return []
