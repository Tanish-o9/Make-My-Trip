...
import { 
  ...
  Trash2, AlertTriangle, ExternalLink, Compass 
} from "lucide-react";
...
export function BookingDetailPage({ bookingId, onNavigate, token }: BookingDetailPageProps) {
  ...
  const handleResendEmail = () => {
    ...
    fetch(`${API_URL}/bookings/${bookingId}/email`, { 
      ...
      .then(() => {
        ...
        loadBookingDetails(); // reload timeline events
      })
      .catch(() => {
  ...
  const handleCancelBooking = () => {
    ...
    fetch(`${API_URL}/bookings/${bookingId}/cancel`, { 
      ...
      .then(data => {
        ...
        loadBookingDetails(); // refresh status and timeline
      })
      .catch(err => {
  ...
  const { booking, ticket, invoice, payment, timeline, vertical } = details;
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              {ticket && (
                <div className="bg-slate-950/40 p-4 rounded-2xl border border-slate-850 space-y-2">
                  <span className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider flex items-center gap-1.5">
                    <Users size={12} /> Passenger / Guest details
                  </span>
            ...
            {/* Live Progress Timeline Panel (Phase 10) */}
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl text-left space-y-4">
            ...
            {/* Google Map coordinates placeholder (Phase 9) */}
